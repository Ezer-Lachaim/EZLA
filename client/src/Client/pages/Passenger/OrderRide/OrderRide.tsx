...
import NumberInput from "../../../components/PassengersNumberInput.tsx";
...
const OrderRide = () => {
  ...
  const [autofilledAddress, setAutofilledAddress] =
    useState<DestinationSourceEnum>(DestinationSourceEnum.Destination);
  ...
  const {
    ...
    defaultValues: {
      ...
      specialRequest: {
        ...
        isBabySafetySeat: user?.specialRequest?.includes(
          specialMap.isBabySafetySeat
        ),
        isChildSafetySeat: user?.specialRequest?.includes(
          specialMap.isChildSafetySeat
        ),
        isHighVehicle: user?.specialRequest?.includes(specialMap.isHighVehicle),
        isWheelChairTrunk: user?.specialRequest?.includes(
          specialMap.isWheelChairTrunk
        ),
        isPatientDelivery: user?.specialRequest?.includes(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (hospitals) {
        const hospitalName =
          hospitals.find((hospital) => hospital.id === user.patient?.hospitalId)
            ?.name || "";
  ...
  return (
    <div className="flex flex-col items-center w-full pb-5">
      <h1 className="mt-0">
        שלום{user?.firstName && ` ${user?.firstName}`}! צריכים הסעה?
      </h1>
      <form
      ...
      >
            ...
            <FormControl>
              ...
              {errors.ride?.origin && (
                <FormHelperText error className="absolute top-full mr-0">
                  {errors.ride.origin.type === "required" &&
                    "יש להזין כתובת מגורים לאיסוף"}
                </FormHelperText>
              )}
            </FormControl>
          ) : (
          ...
          {!user || autofilledAddress === DestinationSourceEnum.Source ? (
            <FormControl>
              ...
              {errors.ride?.destination && (
                <FormHelperText error className="absolute top-full mr-0">
                  {errors.ride.destination.type === "required" &&
                    "יש להזין כתובת מגורים יעד"}
                </FormHelperText>
              )}
            </FormControl>
          ) : (
        ...
        <div className="flex flex-col gap-2">
          <p className="text-sm text-gray-500">מספר הנוסעים</p>
          <FormControl>
          <NumberInput aria-label="מספר הנוסעים" />
          </FormControl>
        </div>
        ...
        <FormControl>
          ...
          {errors.ride?.comment && (
            <FormHelperText error className="absolute top-full mr-0">
              {errors.ride.comment.type === "required" &&
                "יש להזין את מטרת הנסיעה"}
              {errors.ride.comment.type === "maxLength" &&
                "הגעתם למקסימום אורך ההודעה המותר"}
            </FormHelperText>
          )}
        </FormControl>
        ...
        <div className="flex flex-col gap-2">
          ...
          <FormControlLabel
            control={
              <Checkbox {...register("specialRequest.isBabySafetySeat")} />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox {...register("specialRequest.isChildSafetySeat")} />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox {...register("specialRequest.isWheelChairTrunk")} />
            }
          ...
          <FormControlLabel
            control={
              <Checkbox {...register("specialRequest.isPatientDelivery")} />
            }
        ...
        <FormControl>
          ...
          {errors.ride?.firstName && (
            <FormHelperText error className="absolute top-full mr-0">
              ...
              {errors.ride.firstName.type === "minLength" &&
        ...
        <FormControl>
          ...
          {errors.ride?.lastName && (
            <FormHelperText error className="absolute top-full mr-0">
              ...
              {errors.ride.lastName.type === "minLength" &&
        ...
        <FormControl>
          ...
          {errors.ride?.cellphone && (
            <FormHelperText error className="absolute top-full mr-0">
              {errors.ride.cellphone.type === "required" &&
                "יש להזין טלפון נייד"}
              {errors.ride.cellphone.type === "pattern" &&
                "יש להקליד מספר טלפון תקין"}
            </FormHelperText>
          )}
        </FormControl>
          ...
          <div>
            ...
            {errors.isApproveTerms && (
              <FormHelperText error>
                {errors.isApproveTerms.type === "required" &&
                  "יש לאשר קריאת תקנון האתר"}
              </FormHelperText>
            )}
          </div>
        )}
